...
export class ViaHighDensitySolver extends BaseSolver {
  ...
  private _initializeGraph(): boolean {
    ...
    // Create the via graph from XY connections using default viasByNet
    // This is the main entry point as described in the usage guide
    const result = createViaGraphFromXYConnections(
      hgXyConnections,
      defaultViasByNet as ViasByNet,
      {
    ...
    this.tiledViasByNet = result.tiledViasByNet
  ...
  visualize(): GraphicsObject {
    ...
    // Draw via regions as polygons (showing where vias can be placed)
    for (const viaRegion of this.viaRegionInfo) {
      const region = viaRegion.region

      // Only draw via regions that were actually used
      if (this.viaRegionUsage.has(viaRegion.viaRegionId)) {
        const connectedNets = Array.from(
          this.viaRegionUsage.get(viaRegion.viaRegionId) ?? [],
        )
        const color =
          connectedNets.length > 0
            ? (this.colorMap[connectedNets[0]] ?? "gray")
            : "gray"
        ...
        if (region.d.polygon && region.d.polygon.length > 0) {
          ...
          graphics.lines!.push({
            ...
            strokeColor: safeTransparentize(color, 0.4),
            strokeWidth: 0.1,
            strokeDash: "2 2",
            layer: "via-region",
          })

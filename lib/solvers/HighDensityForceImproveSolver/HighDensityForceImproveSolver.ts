...
export type ViaClearancePolicy = {
  sameNetViaClearance: number
  differentNetViaClearance: number
  traceClearance: number
}

export const DEFAULT_VIA_CLEARANCE_POLICY: ViaClearancePolicy = {
  sameNetViaClearance: 0.2,
  differentNetViaClearance: 0.3,
  traceClearance: 0.1,
}
...
const MIN_ALLOWED_CLEARANCE = 0.05
...
const normalizeViaClearancePolicy = (
  policy?: Partial<ViaClearancePolicy>,
): ViaClearancePolicy => ({
  sameNetViaClearance: Math.max(
    policy?.sameNetViaClearance ??
      DEFAULT_VIA_CLEARANCE_POLICY.sameNetViaClearance,
    MIN_ALLOWED_CLEARANCE,
  ),
  differentNetViaClearance: Math.max(
    policy?.differentNetViaClearance ??
      DEFAULT_VIA_CLEARANCE_POLICY.differentNetViaClearance,
    MIN_ALLOWED_CLEARANCE,
  ),
  traceClearance: Math.max(
    policy?.traceClearance ?? DEFAULT_VIA_CLEARANCE_POLICY.traceClearance,
    0,
  ),
})
...
const getViaViaClearance = (
  leftElement: ForceElementBase,
  rightElement: ForceElementBase,
  policy: ViaClearancePolicy,
) =>
...
const resolveClearanceConstraints = (
  ...
  clearancePolicy: ViaClearancePolicy,
  ...
  for (let passIndex = 0; passIndex < passCount; passIndex += 1) {
    ...
    for (let leftIndex = 0; leftIndex < forceElements.length; leftIndex += 1) {
      ...
      for (
        ...
        const targetViaClearance = getViaViaClearance(
          leftElement,
          rightElement,
          clearancePolicy,
        )
        ...
        if (
          Math.abs(separationX) >= targetViaClearance ||
          Math.abs(separationY) >= targetViaClearance
        ) {
          continue
        }
        const distance = Math.hypot(separationX, separationY)
        const penetration = targetViaClearance - distance
...
export const runForceDirectedImprovement = (
  ...
  const viaClearancePolicy = normalizeViaClearancePolicy(
    options?.viaClearancePolicy,
  )
  ...
  for (let stepIndex = 0; stepIndex < totalSteps; stepIndex += 1) {
    ...
    for (let leftIndex = 0; leftIndex < forceElements.length; leftIndex += 1) {
      ...
      for (
        ...
        const viaViaFalloffDistance = Math.max(
          CLEARANCE_FALLOFF_DISTANCE,
          targetViaClearance + 0.1,
        )

        const separationX = leftNode.x - rightNode.x
        const separationY = leftNode.y - rightNode.y
        if (
          Math.abs(separationX) >= viaViaFalloffDistance ||
          Math.abs(separationY) >= viaViaFalloffDistance
        ) {
          continue
        }
        const distance = Math.hypot(separationX, separationY)
...
export class HighDensityForceImproveSolver extends BaseSolver {
  ...
  readonly movementBoundsMargin: number
  readonly viaClearancePolicy: ViaClearancePolicy
  ...
  constructor(params: {
    ...
    this.viaClearancePolicy = normalizeViaClearancePolicy(
      params.viaClearancePolicy,
    )
    this.movementBoundsMargin =
      params.movementBoundsMargin ??
      this.nodeAssignmentMargin +
    ...
    this.stats = {
      ...
      movementBoundsMargin: this.movementBoundsMargin,
      viaClearancePolicy: this.viaClearancePolicy,
  ...
  override _step() {
    ...
    const bounds = getNodeBounds(sampleEntry.node, this.movementBoundsMargin)

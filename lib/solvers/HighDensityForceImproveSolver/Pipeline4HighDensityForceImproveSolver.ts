...
const isPointInsideBounds = (
  ...
  point.x >= bounds.minX + padding - POSITION_EPSILON &&
  point.x <= bounds.maxX - padding + POSITION_EPSILON &&
  point.y >= bounds.minY + padding - POSITION_EPSILON &&
  point.y <= bounds.maxY - padding + POSITION_EPSILON

const isPointInsideNode = (
  point: { x: number; y: number },
  node: NodeWithPortPoints,
  margin = 0,
) => isPointInsideBounds(point, getNodeBounds(node, margin))
...
const getSegmentDistance = (
  ...
  if (segmentLengthSquared <= POSITION_EPSILON) {
    return {
      distance: Math.hypot(pointX - startNode.x, pointY - startNode.y),
      segmentT: 0,
    }
  }

  const toPointX = pointX - startNode.x
  const toPointY = pointY - startNode.y
...
const isPointOnSegment = (
  ...
  Math.abs(getOrientation(ax, ay, bx, by, px, py)) <= POSITION_EPSILON &&
  px >= Math.min(ax, bx) - POSITION_EPSILON &&
  px <= Math.max(ax, bx) + POSITION_EPSILON &&
  py >= Math.min(ay, by) - POSITION_EPSILON &&
  py <= Math.max(ay, by) + POSITION_EPSILON
...
const segmentsIntersect = (
  ...
  if (
    Math.abs(o1) <= POSITION_EPSILON &&
    isPointOnSegment(b1.x, b1.y, a1.x, a1.y, a2.x, a2.y)
  )
    return true
  if (
    Math.abs(o2) <= POSITION_EPSILON &&
    isPointOnSegment(b2.x, b2.y, a1.x, a1.y, a2.x, a2.y)
  )
    return true
  if (
    Math.abs(o3) <= POSITION_EPSILON &&
    isPointOnSegment(a1.x, a1.y, b1.x, b1.y, b2.x, b2.y)
  )
    return true
  if (
    Math.abs(o4) <= POSITION_EPSILON &&
    isPointOnSegment(a2.x, a2.y, b1.x, b1.y, b2.x, b2.y)
  )
    return true
...
const countRouteSelfIntersections = (route: HighDensityRoute) => {
  let count = 0

  for (let i = 0; i < route.route.length - 1; i += 1) {
    const a1 = route.route[i]
    const a2 = route.route[i + 1]
    if (!a1 || !a2 || a1.z !== a2.z) continue

    for (let j = i + 2; j < route.route.length - 1; j += 1) {
      if (j === i + 1) continue
      const b1 = route.route[j]
      const b2 = route.route[j + 1]
      if (!b1 || !b2 || b1.z !== b2.z || a1.z !== b1.z) continue

      const sharesEndpoint =
        (Math.abs(a1.x - b1.x) <= POSITION_EPSILON &&
          Math.abs(a1.y - b1.y) <= POSITION_EPSILON) ||
        (Math.abs(a1.x - b2.x) <= POSITION_EPSILON &&
          Math.abs(a1.y - b2.y) <= POSITION_EPSILON) ||
        (Math.abs(a2.x - b1.x) <= POSITION_EPSILON &&
          Math.abs(a2.y - b1.y) <= POSITION_EPSILON) ||
        (Math.abs(a2.x - b2.x) <= POSITION_EPSILON &&
          Math.abs(a2.y - b2.y) <= POSITION_EPSILON)
...
const scoreRoutes = (
  ...
  for (let leftIndex = 0; leftIndex < forceElements.length; leftIndex += 1) {
    ...
    for (
      ...
      if (distance + POSITION_EPSILON < TARGET_CLEARANCE) {
        ...
        clearancePenalty += TARGET_CLEARANCE - distance
      }
    }
  }
  ...
  for (
    ...
    for (
      ...
      if (!segment) continue
      if (element.rootConnectionName === segment.rootConnectionName) continue
  ...
  for (const route of routes) {
    ...
    selfIntersectionCount += countRouteSelfIntersections(route)

    for (const point of route.route) {
      if (!isPointInsideBounds(point, bounds)) {
        boundaryOverflowCount += 1
      }
    }

    for (const via of route.vias) {
      if (!isPointInsideBounds(via, bounds, route.viaDiameter / 2)) {
        boundaryOverflowCount += 1
      }
    }
  }
...
const areEndpointsStable = (
  beforeRoutes: HighDensityRoute[],
  afterRoutes: HighDensityRoute[],
) =>
  beforeRoutes.every((beforeRoute, index) => {
    const afterRoute = afterRoutes[index]
    if (!afterRoute) return false

    const beforeStart = beforeRoute.route[0]
    const beforeEnd = beforeRoute.route[beforeRoute.route.length - 1]
    const afterStart = afterRoute.route[0]
    const afterEnd = afterRoute.route[afterRoute.route.length - 1]
    ...
    return (
      Math.abs(beforeStart.x - afterStart.x) <= POSITION_EPSILON &&
      Math.abs(beforeStart.y - afterStart.y) <= POSITION_EPSILON &&
      beforeStart.z === afterStart.z &&
      Math.abs(beforeEnd.x - afterEnd.x) <= POSITION_EPSILON &&
      Math.abs(beforeEnd.y - afterEnd.y) <= POSITION_EPSILON &&
      beforeEnd.z === afterEnd.z
    )
  })
...
const shouldAcceptCandidate = (
  ...
  const inputScore = scoreRoutes(bounds, inputRoutes)
  const candidateScore = scoreRoutes(bounds, candidateRoutes)

  if (candidateScore.boundaryOverflowCount > 0) {
    return false
  }

  if (candidateScore.selfIntersectionCount > inputScore.selfIntersectionCount) {
    return false
  }

  const hasPrimaryImprovement =
  ...
  const viaRegression = candidateScore.viaCount > inputScore.viaCount
  if (viaRegression) {
    const majorConflictDrop =
      inputScore.clearanceConflictCount -
        candidateScore.clearanceConflictCount >=
      2
    const strongPenaltyDrop =
      candidateScore.clearancePenalty <= inputScore.clearancePenalty * 0.6
    if (!majorConflictDrop && !strongPenaltyDrop) {
      return false
    }
  }
  ...
  for (let passIndex = 0; passIndex < passCount; passIndex += 1) {
    ...
    for (
      ...
      for (
        ...
        const { distance, segmentT } = getSegmentDistance(
          elementNode.x,
          elementNode.y,
          startNode,
          endNode,
        ...
        if (distance > POSITION_EPSILON) {
          const inverseDistance = 1 / distance
          const closestPointX =
            startNode.x + (endNode.x - startNode.x) * segmentT
          const closestPointY =
            startNode.y + (endNode.y - startNode.y) * segmentT
          directionX = (elementNode.x - closestPointX) * inverseDistance
          directionY = (elementNode.y - closestPointY) * inverseDistance
        } else {
          const normalX = -(endNode.y - startNode.y)
          const normalY = endNode.x - startNode.x
...
export class Pipeline4HighDensityForceImproveSolver extends BaseSolver {
  ...
  override _step() {
    ...
    const attemptedNodeCount = this.activeSampleIndex
    const acceptedNodeCount = accepted
      ? (this.stats.acceptedNodeCount ?? 0) + 1
      : (this.stats.acceptedNodeCount ?? 0)
    const rejectedNodeCount = attemptedNodeCount - acceptedNodeCount
    this.stats = {
      sampleCount: this.sampleEntries.length,
      attemptedNodeCount,
      acceptedNodeCount,
